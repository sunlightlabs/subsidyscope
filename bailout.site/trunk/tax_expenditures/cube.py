...
class Cube:
    ...
    def __add__(self, cube):
        ...
        for attribute in cube.dimensions:
            for value in cube.dimensions[attribute].values:
                for id in cube.dimensions[attribute].values[value]:
                    if not data_attributes.has_key(id):
                        data_attributes[id] = {}
                    
                    data_attributes[id][attribute] = value

        for id in data_attributes:
            
            self.add(data_attributes[id], cube.data[id])
