...
jQuery.fn.multiSelect = function(options)
{
    ...
    */
    ...
    return this.each(function()
    {
        ...
        */
        ...
        {
            var allChecked = (jQuery('option',this).not('[selected]').length == 0);
            
            var li = jQuery('<li class="a9selectall">').appendTo(selectList);
            var checkbox = jQuery('<input type="checkbox" id="multiSelect-options-selectAll-' +id+ '" name="multiSelect-options-selectAll-' +id+'" value="1" title="Select All" ' + (allChecked ? ' checked="checked"': '') + '/>').appendTo(li);
    ...
    */
	...
	var selectCount = $('li:not(.a9selectall) :checkbox:checked', selectList).length+1 - (this.checked ? 1 : 1);
