package flare.vis.operator.layout
{
	...
	public class CirclePackingLayout extends Layout
	{
		...
		public var altLayout:Boolean = true;
		 ...
		 *  Ordered nodes are placed in a spiral starting at the center.
		 */
		public function CirclePackingLayout(spacing:Number=4, altLayout:Boolean=true,
		...
		protected override function layout():void
		{
			...
			if (treeLayout) {
				...
				if(altLayout)
					radius = packCircleAlt(list, list.length);
				else
		...
		private function packCircleAlt(nodes:Array, N:int):Number
		{
			function compareDist(p1:ChainNode, p2:ChainNode):int 
			{
				if (p1.dist < p2.dist) 
        			return 1;
    		...
    		function compareSize(p1:ChainNode, p2:ChainNode):int 
			{
				if (p1.r < p2.r) 
        			return 1;
      			else if (p1.r > p2.r)
    		...
    		for(var iteration:int = 1; iteration < 50; iteration++)
			{
				
				nodes.sort(compareSize);
					...
					for (var j:int = i + 1; j <= 10; j++) 
					{
				  		...
				  		if (i != j) 
				  		{
				    		...
				    		if (d < (r * r) - 0.1 ) 
				    		{
							    ...
							    v.fNormalize();
				      			v.UnaryScalarMultiplication((r - Math.sqrt(d)) * 0.5);
								
								var cjDist:Number = cj.dist;
								var ciDist:Number = ci.dist;
				...
				//Contract
				var damping:Number = 0.01 / Number(iteration);
				
				for (var i:int = 0; i < 25; i++) 
				{
					var c:ChainNode = nodes[i];
					
					v.x = c.x - _b.width / 2;
					v.y = c.y - _b.height / 2;
					v.UnaryScalarMultiplication(damping);
					c.x -= v.x;
					c.y -= v.y;
				}
			}
			...
			for(var iteration:int = 1; iteration < 350; iteration++)
			{
		...
		private function getChainNode(n:NodeSprite):ChainNode
		{
			...
			var cn = new ChainNode(r);
			cn.x = Math.random();
			cn.y = Math.random();
			n.props.chainNode = (cn);
